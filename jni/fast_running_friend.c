...
static int read_config_bool(JNIEnv* env,void* config_obj,Config_var* var, const char* val);
...
static int print_config_bool(JNIEnv* env,void* config_obj,Config_var* var, char* buf, size_t buf_size);
...
Config_var config_vars[] =
{
  ...
  {"external_apps", 0, "Ljava/lang/String;", print_config_str, read_config_str},
  {"use_sys_wifi", 0, "Z", print_config_bool, read_config_bool},
  {0,0,0}
};
...
static int init_config_vars(JNIEnv *env)
{
  ...
  for (v = config_vars; v->name; v++)
  {
    ...
    else if (v->printer == print_config_bool)
...
static int print_config_bool(JNIEnv* env,void* config_obj,Config_var* var,
                            char* buf, size_t buf_size)
{
  jboolean val;
  val = (*env)->GetBooleanField(env,(jobject)config_obj,var->var_id);
  char *c = val ? '1' : '0';
...
static int read_config_bool(JNIEnv* env,void* config_obj,Config_var* var, const char* val)
{
  (*env)->SetBooleanField(env,(jobject)config_obj,var->var_id,
      val && *val == '1');
  return 0;
}
